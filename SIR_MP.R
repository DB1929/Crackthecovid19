### SIR model. THis is using MP incidences for Covid-19. Input required is 
...
MP <- read.table('MP_state_wise_SIR_times_data.csv', header=TRUE, sep=',')
MP$Date <- as.Date(MP$Date,format = "%m/%d/%y")
N <- 85047748 # total population
...
fitted_cumulative_incidence %>%
  ...
  labs(
    ...
    title = "COVID-19 fitted vs observed cumulative incidence, MP",
...
fitted_cumulative_incidence %>%
  ...
  labs(y = "Persons", title = "COVID-19 fitted vs observed cumulative incidence, MP") +
...
death_rate = MP$Deaths[dim(MP)[1]]/MP$Confirmed[dim(MP)[1]]
