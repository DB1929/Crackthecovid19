### SIR model. THis is using kerala incidences for Covid-19. Input required is 
...
kerala <- read.table('Kerala_state_wise_SIR_times_data.csv', header=TRUE, sep=',')
kerala$Date <- as.Date(kerala$Date,format = "%m/%d/%y")
N <- 37961280 # total population
...
fitted_cumulative_incidence %>%
  ...
  labs(
    ...
    title = "COVID-19 fitted vs observed cumulative incidence, kerala",
...
fitted_cumulative_incidence %>%
  ...
  labs(y = "Persons", title = "COVID-19 fitted vs observed cumulative incidence, kerala") +
...
death_rate = kerala$death_cum[dim(kerala)[1]]/kerala$confirmed_cum[dim(kerala)[1]]
